...
`market.yml` remains manual-only. The existing Yahoo collector remains unchanged as the current manual route; this review does **not** clear that route for scheduled collection or establish publication rights. This rights decision itself changed no collector, data, workflow, or test file. Tasks 3 and 4 were subsequently completed as independent maintenance: the divergence pair now rebuilds from the latest committed backtest acquisition and exact shared sealed-household months, and the descriptive anchor report reruns from that committed acquisition. Neither path runs the Yahoo-backed backtest, changes anchors, or enables a Sector Watch schedule.
...
Until every applicable condition is satisfied, the scheduled Sector Watch workflow remains disabled. The completed Task 3 divergence refresh and Task 4 descriptive anchor-validation cadence do not alter or bypass that rights gate.
