...
v5 | Trust      | LAUNCH BLOCKER (board 2026-08-01): replace the "illustrative" 20-year charts on all eight indicator pages — real per-line backfill or an honest "history feed pending" stub; a stranger who spots one fake chart stops believing the real number | todo
v5 | Trust      | LAUNCH BLOCKER (board 2026-08-01): market.html claims anchors are "published in data/market.json" — verified false; publish per-gauge anchor tables in the payload and link the anchor-validation report from market.html + gauge files | todo
v5 | Trust      | LAUNCH BLOCKER (board 2026-08-01): front-page "Integrity: N%" is 100−score wearing an audit label beside Collected/Verified/Sealed — relabel to non-audit language or wire the real integrity-gate result | todo
v5 | Content    | LAUNCH BLOCKER (board 2026-08-01): the 0-100 scale is never defined — plain-English band legend on what-is-ooze.html (thresholds + "what it feels like"), today's reading marked, linked from the hero score chip | todo
v5 | Growth     | LAUNCH BLOCKER (board 2026-08-01): base URL hardcoded across 7 surfaces (sitemap, robots, OG, canonicals, feed, JSON-LD) — one build-time constant so the domain cutover is a single-variable change | todo
v5 | Design     | Board: Sector Watch % color uses ≥0 for green while the jar uses ≥−2% — a −1% row shows green jar + amber number; align thresholds to the published state rule | todo
v5 | Design     | Board: divergence chart illegible at 360px (axis text ~4px) and inverts color grammar (green=ward, off-token blue=household) — overflow-x container + one --ward token, household keeps ooze green | todo
v5 | Website    | Board: statically stamp indicator pages + market.html at build time (score, value, prose) so pre-JS HTML never reads "—/100 OFFLINE" — crawlers and previews see the number | todo
v5 | Design     | Board: boot clearance auto-skips by default (score visible <1s); ceremony becomes an optional replay | todo
v5 | Growth     | Board: share block (Copy Report / Post to X) on market.html + all indicator and gauge pages with per-page reading pre-filled; per-page OG cards that bake the number into the image | todo
v5 | Website    | Board: sitemap.xml missing all six market/* gauge pages + lessons; index.html and market.html missing canonical tags | todo
v5 | Website    | Board: lessons wing folds into the facility — course.css points at lab.css tokens, OOZE ACADEMY wordmark links home, standard footer appended, "Today's Reading" link on every lesson | todo
v5 | Trust      | Board: policies.html + notes.html cover half the facility — add Ward M section (house rules apply) and disclose the two zero-weight aux jar lines so "complete recipe" is true | todo
...
v5 | Content    | Board: Ward M copy rewrite for civilians — engineering internals (NFCI, quote.close transport, anchor provenance) move to a linked methodology note; gauge pages gain the can-glass instrument | todo
